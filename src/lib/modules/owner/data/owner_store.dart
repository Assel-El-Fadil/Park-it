...
class OwnerStoreController extends Notifier<OwnerStoreState> {
  ...
  OwnerStoreState build() {
    ...
    ref.listen(currentUserProvider, (prev, next) async {
      final ownerId = int.tryParse(next?.id ?? '');
    ...
    final initialOwnerId = int.tryParse(ref.read(currentUserProvider)?.id ?? '');
  ...
  Future<void> _loadForOwner(int ownerId) async {
    ...
    final pricingRules =
        await repo.getDynamicPricingRulesForSpotIds(spotIds);
  ...
  Future<void> updateReviewOwnerReply({
    ...
    await ref.read(ownerRepositoryProvider).updateReviewOwnerReply(
          reviewId: reviewId,
  ...
  Future<void> setDynamicPricingRules({
    ...
    final ownerId = _loadedOwnerId;
    if (ownerId == null) return;
    await ref.read(ownerRepositoryProvider).setDynamicPricing(
          spotId: spotId,
